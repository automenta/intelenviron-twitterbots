 ...
 */
package afxdeadcode.netention.feed;

import afxdeadcode.netention.Detail;
import afxdeadcode.netention.Mode;
...
public class TwitterChannel implements Serializable {
    ...
    public static List<Detail> getTweets(Query query) throws Exception {
        ...
        for (Object o : result.getTweets()) {
            ...
            Detail d = new Detail(tweet.getText(), Mode.Real).withID("twitter/" + tweet.getId());
            
            d.setWhen(tweet.getCreatedAt());   
            d.setFrom(getTwitterAgent(tweet.getFromUser()));
